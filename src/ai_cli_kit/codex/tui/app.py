...
TUI_ACTION_NOTES = {
    ...
    "clean_archived": ["删除已归档会话，并清理 index / threads / Desktop 状态中的残留引用。", "执行前需要输入 DELETE 二次确认。"],
    "clean_archived_dry": ["只预览将删除哪些已归档会话及其残留引用。"],
    ...
    "switch_provider_dry": ["只预览 Desktop 会话切换到当前 provider 的结果，不写入任何文件。"],
...
def build_tui_menu_actions() -> List[TuiMenuAction]:
    return [
        ...
        TuiMenuAction("repair_session_history", "8", "修复指定对话历史", "repair", ("repair-session-history", "<session_id>")),
        ...
        TuiMenuAction("switch_provider_dry", "s", "模拟切换到当前 provider（Dry-run）", "repair", ("switch-provider", "--dry-run"), is_dry_run=True),
        ...
        TuiMenuAction("clean_archived", "k", "清理已归档会话", "repair", ("clean-archived", "--yes"), is_dangerous=True),
        TuiMenuAction("clean_archived_dry", "v", "模拟清理已归档会话", "repair", ("clean-archived", "--dry-run"), is_dangerous=True, is_dry_run=True),
...
class ToolkitTuiApp:
    ...
    def _fit_lines_to_screen(self, lines: List[str]) -> List[str]:
        ...
        tail_rows = min(4, max(1, visible_rows // 3))
        head_rows = visible_rows - tail_rows - 1
        folded_notice = style_text("... 窗口高度不足，中间内容已折叠；可放大终端窗口继续查看 ...", Ansi.DIM, Ansi.YELLOW)
        return [*lines[:head_rows], folded_notice, *lines[-tail_rows:]]
    ...
    def _render_section_page(self, section_index: int, action_offset: int) -> None:
        ...
        max_visible_actions = max(1, screen_height - reserved_rows - 9)
    ...
    def _confirm_dangerous_action(self, cli_args: Sequence[str]) -> bool:
        ...
        is_archived_cleanup = bool(cli_args) and cli_args[0] == "clean-archived"
        action_description = (
            "Clean Archived 会删除已归档会话及其索引引用。"
            if is_archived_cleanup
            else "Clean 会删除旧版无标记副本文件。"
        )
        affected_scope = "archived_sessions 文件及 index / threads / Desktop 状态引用" if is_archived_cleanup else "旧版无标记 clone 文件"
        info_lines = [
            style_text(f"{glyphs().get('danger', '!!')} 【危险】", Ansi.BOLD, Ansi.RED) + f" {action_description}",
            f"{style_text('执行方式', Ansi.DIM)} : 直接在 TUI 中执行",
            f"{style_text('影响范围', Ansi.DIM)} : {affected_scope}",
            "",
            "确认方式：输入 DELETE 并回车。",
            "取消方式：直接回车。",
        ]
